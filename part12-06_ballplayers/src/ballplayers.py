...
def most_goals(ballplayer):
    return max(ballplayer, key=lambda x: x.goals).name

def most_points(ballplayer):
    player = max(ballplayer, key=lambda x: x.goals + x.passes)
    return player.name, player.number

def least_minutes(ballplayer):
    return min(ballplayer, key=lambda x: x.minutes)
